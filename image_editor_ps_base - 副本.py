...
class ImageEditorPS(QMainWindow):
    ...
    def apply_brightness_contrast(self):
        ...
        try:
            ...
            # 计算调整参数
            brightness_factor = brightness / 100.0 * 128  # 限制亮度调整范围
            contrast_factor = 1.0 + contrast / 100.0  # 对比度范围0.5-1.5
            
            # 处理彩色或灰度图像
            if len(self.image_data.shape) == 3:  # 彩色
                adjusted = np.zeros_like(self.image_data, dtype=np.float32)
                for c in range(3):
                    channel = self.image_data[..., c].astype(np.float32)
                    # 应用对比度和亮度(基于当前图像状态)
                    channel = (channel - 128) * contrast_factor + 128 + brightness_factor
                    # 裁剪到0-255范围
                    adjusted[..., c] = np.clip(channel, 0, 255)
            else:  # 灰度
                adjusted = self.image_data.astype(np.float32)
                # 应用对比度和亮度(基于当前图像状态)
                adjusted = (adjusted - 128) * contrast_factor + 128 + brightness_factor
                # 裁剪到0-255范围
